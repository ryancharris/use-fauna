...
export default function useUpdateDocument(db: faunadb.Client, collectionName: string, refId: string, data: object) {
  ...
  useEffect(() => {
    const request = db.query(
      q.Replace(q.Ref(q.Collection(collectionName), refId), { data })
    );
